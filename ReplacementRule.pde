...
class ReplacementRule {
  private int minAge;
  private int maxAge;
  HashMap<Agent, Integer> maxAgeAgents = new HashMap<Agent, Integer>();
  AgentFactory fac;
  ...
  */
  public boolean replaceThisOne(Agent a) {
    ...
    if (maxAgeAgents.containsKey(a) == false){
      Random r = new Random();
      int e = (minAge + r.nextInt(maxAge+1 - minAge));
      maxAgeAgents.put(a, e);
    }
    else {
      if (a.getAge() > maxAgeAgents.get(a)) {
        a.setAge(maxAge+1);
        return true;
      }
    }
